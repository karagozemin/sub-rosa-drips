...
import { runCommand, type CommandContext } from "@sub-rosa/command";
...
async function fixtureMain(repoRoot: string) {
  const fixturePath = resolve(repoRoot, "services/receipt-cli/src/fixtures/golden.json");
...
async function testnetMain(ctx: CommandContext) {
  ...
  diagnostics.info("rpc", "rpc:     ", { "RPC_URL_0": rpcUrl });
...
runCommand({
  name: "services.auction-template.sealed-auction",
  description: "Runnable sealed-auction integration template for Sub Rosa",
  options: {
    fixture: {
      type: "boolean",
      description: "Run in offline fixture mode",
    },
  },
  async run(ctx) {
    const isFixture = Boolean(ctx.args.fixture) || ctx.env.FIXTURE === "1";
    if (isFixture) {
      await fixtureMain(ctx.repoRoot);
    } else {
      await testnetMain(ctx);
    }
    return 0;
  },
});
