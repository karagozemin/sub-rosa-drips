...
async function writeAuditorTrace(path: string, trace: unknown, repoRoot: string) {
...
runCommand({
  name: "services.keeper.lifecycle-e2e",
  description: "Full live testnet lifecycle proof",
  async run(ctx) {
    const operatorSecret = reqEnv("OPERATOR_SECRET", ctx.env);
    const bidder1Secret = reqEnv("BIDDER1_SECRET", ctx.env);
    const bidder2Secret = reqEnv("BIDDER2_SECRET", ctx.env);
    ...
    const operator = new SubRosaClient({ rpcUrl, networkPassphrase: network, contractId, secretKey: operatorSecret });
    const now = clock.nowSeconds();
    const commitDeadline = now + 90;
    const revealRound = Math.ceil((now + 120 - DRAND_GENESIS) / DRAND_PERIOD);
    const tReveal = DRAND_GENESIS + DRAND_PERIOD * revealRound;
    const revealDeadline = tReveal + 30;
    const auditor = generateAuditorKeypair();

    diagnostics.info("2-8-createround-highestbid", "\n[2/8] createRound (HighestBid)…", { "value1_0": {
      revealRound,
      timeUntilR: `${tReveal - now}s`,
      commitDeadline: `${commitDeadline - now}s`,
      revealDeadline: `${revealDeadline - now}s`,
    } });
    const roundId = await operator.createRound({
      itemRef: sha256("sub-rosa://lifecycle-e2e/test-asset"),
      revealRound,
      commitDeadline,
      revealDeadline,
    ...
    async function commitBid(secret: string, value: bigint, escrow: bigint, who: string) {
      ...
      const client = new SubRosaClient({ rpcUrl, networkPassphrase: network, contractId, secretKey: secret });
    ...
    diagnostics.info("locked-usdc", "    locked USDC:", { "value1_0": {
      operator: usdc(locked.op), bidder1: usdc(locked.b1), bidder2: usdc(locked.b2), contract: usdc(locked.contract),
    } });
    if (before.b1 - locked.b1 !== E1) fail(`bidder1 escrow mismatch: delta=${before.b1 - locked.b1} expected=${E1}`);
    if (before.b2 - locked.b2 !== E2) fail(`bidder2 escrow mismatch: delta=${before.b2 - locked.b2} expected=${E2}`);
    if (locked.contract - before.contract !== E1 + E2) fail("contract escrow != E1 + E2");
    diagnostics.info("both-escrows-locked-in-contract-total-e1-e2", "    ✔ both escrows locked in contract (total = E1 + E2)");

    diagnostics.info("4-8-keeper-waits-for-r-opens-reveal-with-real-signature", "\n[4/8] keeper waits for R, opens reveal with real signature, reveals both…");
    const keeperSdk = new SubRosaClient({ rpcUrl, networkPassphrase: network, contractId, secretKey: keeperSecret });
