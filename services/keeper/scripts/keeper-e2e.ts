...
runCommand({
  name: "services.keeper.keeper-e2e",
  description: "Live keeper end-to-end proof",
  async run(ctx) {
    const operatorSecret = reqEnv("OPERATOR_SECRET", ctx.env);
    const bidderSecret = reqEnv("BIDDER_SECRET", ctx.env);
    const keeperSecret = reqEnv("KEEPER_SECRET", ctx.env);
    const wasmHash = reqEnv("WASM_HASH", ctx.env);
    const usdc = reqEnv("USDC_SAC", ctx.env);
    ...
    const bidder = new SubRosaClient({ rpcUrl, networkPassphrase: network, contractId, secretKey: bidderSecret });
    ...
    const reader = new SubRosaClient({ rpcUrl, networkPassphrase: network, contractId, publicKey: keeperKp.publicKey() });
