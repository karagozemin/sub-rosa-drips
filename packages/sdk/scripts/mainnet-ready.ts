...
async function main() {
  ...
  const dryRun =
    process.argv.includes("--dry-run") || env.MAINNET_DRY_RUN === "1";
  const withBalances = process.argv.includes("--with-balances");
  const strict = process.argv.includes("--strict");

  const rpcUrl = env.RPC_URL ?? MAINNET_ARTIFACTS.rpcUrl;
  const networkPassphrase =
    env.NETWORK_PASSPHRASE ?? MAINNET_ARTIFACTS.networkPassphrase;
  const contractId =
    env.ROUND_CONTRACT_ID ?? MAINNET_ARTIFACTS.contractId;

  const operatorAccount = env.OPERATOR_SECRET
    ? Keypair.fromSecret(env.OPERATOR_SECRET).publicKey()
    : undefined;
  const keeperAccount = env.KEEPER_SECRET
    ? Keypair.fromSecret(env.KEEPER_SECRET).publicKey()
    : undefined;
  const bidderAccount = env.BIDDER_SECRET
    ? Keypair.fromSecret(env.BIDDER_SECRET).publicKey()
    : undefined;
  ...
  const reader = dryRun
    ...
    : new SubRosaClient({
        ...
        contractId,
        publicKey:
          env.MAINNET_READER_PUBKEY ?? DEFAULT_READER_PUBKEY,
      });
